...
resource "aws_alb_listener_rule" "domain_and_url_https" {
  ...
  action {
    type             = "forward"
    target_group_arn = var.app_target_group_arn == "" ? aws_alb_target_group.app[0].arn : var.app_target_group_arn
  }
...
resource "aws_alb_listener_rule" "url_https" {
  count        = var.setup_listener_rule && var.enable_https_rules && var.setup_target_group ? ceil(length(var.urls) / 5) : 0
  listener_arn = var.alb_listener_https_arn
